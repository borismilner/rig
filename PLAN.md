...
|---|---|---|---|
| 1 | **Estate identity in the protocol.** A name and a role (`production` / `development`) an agent can READ | §14, and the wire | **ON THE WIRE 2026-09-11 as `rig.estate` (`1177f80`), and NOT YET REACHABLE FROM A TERMINAL.** The method exists precisely because a terminal never handshakes, and `rig` has no verb for it yet - so the caller the design is for still cannot reach it. **Half done, and the half that is missing is the half Boris asked for.** Assigned |
