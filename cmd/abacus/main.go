...
// and the kit is what has to bend. cmd/ledger is pull-report's shape - 16
// tables of text in a five-part page. This one is dispatch's, which the census
// puts at ONE table and one call site, and section 5h names as the hard case:
// clinic at 143 tables is where a program obviously wants a table element, and
// proving the obvious case proves very little.
//
// Modelled on frontend/src/pages/CiBoard.tsx, read rather than remembered.
// What is there: seven columns, a row of pill toggles for a filter, three
// skeleton bars while it loads, and an empty message whose TEXT DEPENDS ON THE
// FILTER. Almost none of its cells are text - a PR is a link with an icon, an
// assignment is a pill, a state is a chip, builds are a matrix of squares.
//
// It declares TWO elements, not three, and the one it leaves out is the
// finding. See the element list in declaration() below.
//
...
func run() error {
	...
	// probe, so there is nothing to route on yet. Same as ledger's, and that
	// sameness is deliberate - what these two fake applications are for is
	// disagreeing about the PAGE, not about the wire.
...
func declaration(id, paneURL string) *rigv1.Declaration {
	return &rigv1.Declaration{
		...
		// TWO elements, and the one that is missing is this program's whole
		// point.
		//
		// dispatch's filter is a row of pill toggles - all / failing /
		// running - and rigToolbar cannot be one. It always creates a search
		// input (kit.js: the input is unconditional, only the select is
		// optional) and it has no segmented control at all. Forcing a search
		// box onto a board that filters by three named states would be a fake
		// application written to fit the kit, which section 5h says proves
		// nothing. So the toolbar is not used, and the pills are the
		// program's own markup.
		//
		// That is the second adopter doing its job: it takes 2 of 3, and the
		// one it refuses is the one already recorded as the kit's worst bend.
		// R1 keeps the inventory open while the only adopters are fake, so a
		// segmented control is a candidate for a fourth element rather than a
		// defect in this program.
		Elements: []string{"rigPanel", "rigTable"},
