 ...
 * that come apart as soon as a cell is a thing rather than a string. abacus's
  ...
  function render() {
    ...
    if (r.length === 0) {
      ...
      const td = el(
        "td",
        "rig-empty",
        // emptyText may be a function, because dispatch's board says
        // something different for each of its three filters ("No failing
        // runs.", "Nothing running right now.", "No linked PRs yet.") and a
        // single string cannot. The search filter keeps its own message: that
        // one is about what was typed, which the caller did not choose.
        filter
          ? `Nothing matches "${filter}".`
          : (typeof opts.emptyText === "function"
              ? opts.emptyText()
      ...
      r.slice(0, opts.limit || 5000).forEach((row) => {
        ...
        cols.forEach((c) => {
          const td = el("td");
          // c.el returns a NODE, and it replaced a c.html that returned a
          // string. Nothing used c.html, it was not in the documented option
          // list, and it assigned innerHTML - so the first adopter with a cell
          // that is not text would have reached for an injection sink built
          // into rig's own element and interpolated a repo name or a title
          // into it. The second fake application is what found that: every
          // interesting column on dispatch's board is a link, a chip or a
          // matrix, so "a cell can be a thing rather than a string" is the
          // normal case for that shape and not an escape hatch at all.
          if (c.el) {
            const node = c.el(row);
