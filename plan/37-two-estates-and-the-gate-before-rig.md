...
| 3 | **Build and semantic skew is detected, not discovered.** A client built from the development tree talking to the production daemon is refused or warned | §21, and **the wire** | **BUILT AND DEMONSTRATED 2026-09-14 (`7283ce6`), BUILD SKEW ONLY.** Ruled by Boris: the CLI WARNS and never refuses - see "Precondition 3's rule" below. **Demonstrated on live daemons**: a v0.5.0 rig against a v0.4.0 `production` estate warns, a matching build is silent, an unstamped rig says not checked, a daemon from before `rig.estate` reads as older; exit codes 0 and `--json` stdout byte-identical in every case. Cost measured at ~60us on a 2.4ms invocation. **The semantic generation is not covered.** **RE-MEASURED 2026-09-11, and it is TWO different gaps wearing one row - see below.** For a PROGRAM the fields already exist and nothing reads them. For a TERMINAL or an AGENT there is no handshake to carry them at all. **Batched with row 1 as one wire change** |
...
#### Why precondition 3 waited, and what it waited for
...
#### Precondition 3's rule, RULED BY BORIS 2026-09-14: warn, never refuse

**Chosen from three options with their costs.** Refusing writes on production and
refusing any mismatch were declined. `DECISIONS.md` 2026-09-14 carries the record.

| | The rule |
|---|---|
| **who checks** | the `rig` CLI, on every verb that reaches a daemon. **Shell completion is exempt**: it runs while a key is held and has nowhere to say anything |
| **what it compares** | its own stamped build (`main.version`) against `EstateResponse.daemon_version`. **Build only** |
| **when they match** | silent |
| **when they differ** | **one line on stderr, unprompted**, naming both builds and the estate reached. **Exit code and stdout unchanged**, so `--json` consumers see no difference |
| **an unstamped side** (`dev`, or an empty `daemon_version`) | reported as **not comparable**, never as a match. Zero means nothing was said (§21) |
| **a daemon with no `rig.estate`** (`CODE_NOT_FOUND`) | reported as **older than the check**, which is itself skew |
| **the check cannot complete** | reported as not comparable, and the verb proceeds. A skew check must never be the reason a call fails |

**The case it exists for:** `build/rig` from the development tree, run with no
explicit `XDG_RUNTIME_DIR`, reaches PRODUCTION, because precondition 5 gives
production the default. That pair always differs by build string.

**What it does not cover, so nobody reads it as more:**

- **the semantic generation** - still the open measurement above
- **agents on the MCP socket** - the server is inside rigd, so there is no second build to compare
- **registered programs** - a different wire major rides the proto path and cannot connect at all
- **two `-dirty` builds with one string** compare equal. That pair only occurs development against development, which is not what this precondition guards

**The cost, named:** one extra round trip per CLI invocation.
