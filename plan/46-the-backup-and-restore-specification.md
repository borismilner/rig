...
| 9 | `cmd/rig/layering_test.go` stays green, and `go list -deps ./cmd/rig \| grep -c modernc` prints 0 | import `internal/record` from `internal/backup` in a detached copy: the layering test goes red |
