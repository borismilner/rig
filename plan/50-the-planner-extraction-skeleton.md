...
| | Test | Command |
|---|---|---|
| **A** | **the planner imports nothing of rig but the stub and the stub's closure** (decision 1) | `go list -deps ./... \| grep github.com/borismilner/rig \| grep -v -E '/client$\|/internal/paths$\|/internal/wire$\|/proto/rig/v1$'` prints **nothing** in docket's module. **At move 3, before the stub is needed, the plain `grep -c` prints 0** |
